...
<?php session_start() ?>
<?php
$coordinator_user_id =  $_SESSION["coordinator_user_id"];
if (!isset($coordinator_user_id)) {
    header("location:login.php");
}
if (isset($_POST['submitCoordinatorData'])) {
    $coordinator_name = $_SESSION['username'];
    $coordinator_id = $_SESSION['$coordinator_user_id'];
    $coordinator_title = mysqli_real_escape_string($conn, $_POST['$coordinator_title']);
    ...
    $coordinator_title = mysqli_real_escape_string($conn, $_POST['$coordinator_title']);
    $coordiator_role_id = $_SESSION['$coordiator_role_id'];

    $image = $_FILES['image']['name'];
    $image_size = $_FILES['image']['size'];
    $tmp_name = $_FILES['name']['tmp_name'];
    $img_path = 'uploads/coordinators/' . $image;

    if (!empty($image)) {
        if ($image_size > 2000000) {
            $message[] = 'image file size is too large';
        } else {
            $coordinatorProfile = "INSERT INTO coordinator (coordinator_title, coordinator_name, church ,country, image) VALUES ('$coordinator_title', '$coordinator_name', '$church' ,'$country', '$image')";
            mysqli_query($conn, $coordinatorProfile);
            ...
            <div class="col-12 col-xl-8">
                <div class="card rounded-4 border-top border-4 border-primary border-gradient-1">
                    <div class="card-body p-4">
                        ...
                        <form method="post" action="uploadCoordinatorProfile.php" enctype="multipart/form-data" class="row g-4">

                            <div class="col-md-6">
                                <label for="input1" class="form-label">Title</label>
                                <input type="text" name="coordinator_title" class="form-control" id="input1" placeholder="Pastor">
                            </div>

                            <div class="col-md-6">
                                <label for="input2" class="form-label">Coordinator Name</label>
                                <input type="text" name="coordinator_name" class="form-control" id="input2" placeholder="Stephen Chad Ethan">
                            </div>

                            <div class="col-md-12">
                                <label for="input3" class="form-label">Coordinator Church</label>
                                <input type="text" name="church" class="form-control" id="input3" placeholder="Christ Embassy">
                            </div>


                            <div class="col-md-12">
                                <label for="input7" class="form-label">Coordinator Country</label>
                            ...
                            <div class="col-md-12">
                                <div class="d-md-flex d-grid align-items-center gap-3">
                                    <button type="button" name="submitCoordinatorData" class="btn btn-grd-primary px-4">Upload Profile</button>
